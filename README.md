...
- 生图能力**全局开关**（设置/供应商表单）：开启后模型可通过 MCP 工具与内置 image_gen 生图（账号池，无需订阅）；关闭后移除 MCP 注册与工具注入，模型回到无生图能力的原始状态，即时生效
- 生图作为 Agent 原生 MCP 工具：通过 Grok CLI 原生 `[mcp_servers]` 注册（Grok CLI 自己加载），对话中模型可自行调用 `generate_image` 撰写提示词，并自主选择模式（快速 / 标准 / 高清）与宽高比（1:1、16:9、9:16、4:3、3:4、3:2、2:3、4:5、21:9 等），结果直接回传对话；同一 MCP 服务器兼容任何支持 MCP 的 Harness
- OpenAI 兼容 Harness 也能生图：本地代理会在 `chat/completions` 请求中自动注入 `generate_image` 工具声明，模型发起生图调用时代为执行（账号池）并把图片内嵌（base64）返回，任何支持 function calling 的平台无需配置即可使用
- 内置 `image_gen` 自动走账号池：Grok Auth 本地代理下，模型的内置生图工具请求会被本地代理接管，用账号池生图，官方账号无额度也能用（无需订阅）

...
(function () {
  ...
  const DICT = {
    ...
    "无匹配项": "No matches",
        "Cloudflare 通行证（cf_clearance，过期较快；点「采集通行证」自动获取）": "Cloudflare clearance (cf_clearance, expires quickly; click Harvest to fetch)",
    "采集通行证": "Harvest clearance",
    "也可从本机浏览器复制 grok.com 的 cf_clearance": "Or paste the cf_clearance cookie of grok.com from your browser",
    "启动可见浏览器访问 grok.com 过盾并采集 cf_clearance（必要时可手动点击验证码）": "Opens a visible browser to pass the grok.com challenge and harvest cf_clearance (click the captcha manually if needed)",
    "Web 通道号池（思考）": "Web channel pool (thinking)",
    "grok2api 式网页通道：SSO cookie 调 grok.com 网页 API，对外提供 /web/v1 OpenAI 兼容端点，思考以 reasoning_content 返回。与 CLI OAuth 号池并列。": "grok2api-style web channel: SSO cookies call grok.com's web API and serve an OpenAI-compatible /web/v1 endpoint; thinking is returned as reasoning_content.",
    "启用 /web/v1 端点": "Enable /web/v1 endpoint",
    "出口代理（建议与号池一致：cf_clearance 绑定注册时的出口 IP）": "Egress proxy (keep consistent with the pool: cf_clearance is bound to the registration IP)",
    "保存 Web 通道设置": "Save web channel settings",
    "从注册机导入 Cookie": "Import cookies from registrar",
    "粘贴导入 SSO…": "Paste SSO…",
    "接入内置工作台": "Connect to built-in workbench",
    "把 Web 通道注册为内置工作台的供应商 Profile（native 引擎自动显示思考）": "Register the web channel as a provider profile for the built-in workbench (native engine shows thinking automatically)",
    "尚未导入 Web 通道账号。": "No web channel accounts yet.",
    "Web 通道 Base URL": "Web channel Base URL",
    "Web 通道 API Key": "Web channel API Key",
    "暂无账号。点击「从注册机导入 Cookie」把注册机的 SSO 快照导入本通道。": "No accounts. Click \"Import cookies from registrar\" to pull the registrar's SSO snapshots into this channel.",
    "每行一枚 SSO cookie，或用逗号分隔": "One SSO cookie per line, or comma-separated",
    "健康": "Healthy",
    "限流中": "Rate limited",
    "被屏蔽": "Blocked",
    "已停用": "(disabled)",
    ...
    "解释这个项目": "Explain this project",
    "写一个自动化脚本": "Write an automation script",
    "生成一张配图": "Generate an illustration",
    "解释这个项目的结构和主要功能，列出关键文件。": "Explain this project's structure and main features; list the key files.",
    "帮我写一个自动化脚本，完成一个小而完整的任务。": "Write an automation script that completes one small, self-contained task.",
    "生成一张配图：": "Generate an illustration: ",
